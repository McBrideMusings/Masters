...
namespace PlayByPierce {
	public enum StateTransition { Safe, Overwrite, }
  ...
  /// class description
  /// </summary>
	public class MonoStateMachine<T> : IMonoStateMachine where T : struct, IConvertible, IComparable {
		public event Action<T> Changed;

		private MonoStateMachineRunner engine;
		...
		public MonoStateMachine(MonoStateMachineRunner engine, MonoBehaviour component)
		{
			this.engine = engine;
		...
		public static MonoStateMachine<T> Initialize(MonoBehaviour component)
		{
			var engine = component.GetComponent<MonoStateMachineRunner>();
			if (engine == null) engine = component.gameObject.AddComponent<MonoStateMachineRunner>();
		...
		/// <returns>A valid stateMachine instance to manage MonoBehaviour state transitions</returns>
		public static MonoStateMachine<T> Initialize(MonoBehaviour component, T startState)
		{
			var engine = component.GetComponent<MonoStateMachineRunner>();
			if (engine == null) engine = component.gameObject.AddComponent<MonoStateMachineRunner>();
